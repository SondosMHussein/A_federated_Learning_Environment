...
def federated_averaging(server, clients ,C_Loss):
    target = {name:value.to(device) for name,value in server.named_parameters()}
    sources = []
    clinetLoss= np.array(list(C_Loss.values()))

    clinetLoss_Standard_dev = np.std(clinetLoss)
    clinetLoss_mean = np.mean(clinetLoss)
    
    ClientType = {}
    for client_name,model in clients.items():
        if (C_Loss[client_name] <= clinetLoss_mean +  clinetLoss_Standard_dev ) and  (C_Loss[client_name] >= clinetLoss_mean -  clinetLoss_Standard_dev ) :
            ClientType[client_name] = 0
            source = {name:value.to(device) for name,value in model.named_parameters()}
            sources.append(source)

        elif (C_Loss[client_name] <= clinetLoss_mean + 2* clinetLoss_Standard_dev )and  (C_Loss[client_name] >= clinetLoss_mean - 2* clinetLoss_Standard_dev ) :
            ClientType[client_name] = 1
           random_num = random.randint(1,100)
			if (random_num > 50):
				source = {name:value.to(device)*1 for name,value in model.named_parameters()}
				sources.append(source)
			else:
				#not calculate malicious
		else:
			Clients_Type[client_name] = 2
    print(ClientType)
